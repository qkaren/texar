...
def seq2seq_pad_concat_convert(xy_batch, eos_id=2, bos_id=1):
    ...
    f_block = np.pad(f_block, ((0, 0), (0, 1)), 'constant',
                     constant_values=0)
    for i_batch, seq in enumerate(f_seqs):
        f_block[i_batch, len(seq)] = eos_id

    # Add BOS in target language
    y_in_block = np.pad(y_block, ((0, 0), (1, 0)), 'constant',
                        constant_values=bos_id)

    return x_block, y_in_block, y_out_block, f_block
