...
case ${encoder} in
    ...
    'bpe'):
        cat ${train_src} ${train_tgt} ${train_fact} | learn_bpe -s ${vocab_size} > ${out}/data/bpe-codes.${vocab_size}
        ...
        apply_bpe -c ${out}/data/bpe-codes.${vocab_size} < ${train_fact} > $out/data/train.${fact_language}.bpe
        apply_bpe -c ${out}/data/bpe-codes.${vocab_size} < ${valid_fact} > ${out}/data/valid.${fact_language}.bpe
        apply_bpe -c ${out}/data/bpe-codes.${vocab_size} < ${test_fact} > ${out}/data/test.${fact_language}.bpe
        cp ${test_tgt} ${out}/test/test.${tgt_language} ;;
esac
